...
import com.ibeifeng.senior.usertrack.spark.util._
...
object UserVisitActionAnalyzerSpark {
  def main(args: Array[String]): Unit = {
    ...
    // 二、上下文的创建
    // 2.1 参数的获取 Constants.SPARK_APP_NAME_SESSION == "UserVisitSessionAnalyzeSpark_"
    ...
    val actionRDD: RDD[UserVisitSessionRecord] = this.getActionRDDByFilter(spark, taskParam)
    ...
    val sessionID2LengthRDD: RDD[(String, Long)] = sessionID2RecordsRDD.map {
      case (sessionID, records) => {
        ...
        val actionTimestamps: Iterable[Long] = records.map(record => {
          ...
          val timestamp: Long = DateUtils.parseString2Long(actionTime)
          timestamp
        })
    ...
    // 计算各个小时段各个会话的会话长度
    //sessionID2RecordsRDD (abbd0359-910c-4cf8-8b5a-59fd04d16810,Iter(record1,record2,record3...))
    val dayAndHour2SessionLengthRDD: RDD[((String, Int), Long)] = sessionID2RecordsRDD.flatMap {
      case (sessionID, records) => {
        ...
        val dayAndHourAndSessionID2TimestampIter: Iterable[((String, Int, String), Long)] = records.map(record => {
          ...
          //2018-05-18,267,23ff0ab5-f4ca-46d9-b5e3-7a1cb0dfa781,3,2018-05-18 05:29:14,,,,,,,,1  =》 （（2018-11-21, 14, 3ff0ab5-f4ca-46d9-b5e3-7a1cb0dfa781），1283283283833）
          ((day, hour, record.sessionId), timestamp)
        })

        // 2. 计算各个时间段各个会话中的会话长度
        val dayAndHour2LengthIter: Map[(String, Int), Long] = dayAndHourAndSessionID2TimestampIter
          .groupBy(_._1) //((2018-05-18,05,s01 ),Iter(((2018-05-18,05,s01 ),1231232),120888,12321321))
          .map {
    ...
    //dayAndHour2SessionLengthRDD  ((20181121,14),20000) ((20181121,14),300000)
    //每个小时内的会话个数
    val preDayAndHourOfSessionCount = dayAndHour2SessionLengthRDD
      .map(tuple => (tuple._1, 1))
      .reduceByKey(_ + _) //((20181121,14),368)
      .collect()
    ...
    val sampleSessionDataSavePath = s"/spark/project/session_sample/task_${taskID}"
    ...
    /**
      ...
      */
    //sessionID2RecordsRDD: RDD[(String, Iterable[UserVisitSessionRecord])]
    val top10CategoryIDAndCountRDD: RDD[(String, (Int, Int, Int))] = sessionID2RecordsRDD
      .flatMap {
        case (sessionID, records) => {
          // 从records迭代器中获取各个操作触发的品类id：Iterable[(String, Int)] ==> Iterable[(品类id, 操作类型的标记信息)]
          ...
          // 返回值 Iterable[((String, Int), Int)]  Iterable[((品类id, 品类标志), 1)]
          iter.map(t => (t, 1)) //((品类id，点击0)，1)  ((品类id,下单1),1)  ((品类id,支付2),1)
        }
      }
      .reduceByKey(_ + _) //((String, Int), Int) = ((品类id, 操作标志), 点击次数)
    ...
    //比如：手机被点击的次数最多100万次，由50000用户（近似为session）点击，求50000用户中点击最所的10个用户
    // 耐克鞋被点击次数第2多，比如是90万次，由30000用户（近似为session）点击，求30000用户中点击最所的10个用户
    /**
    ...
    val top10CategoryIDList: Array[String] = top10CategoryIDAndCountRDD.collect().map(t => t._1)
    ...
    val preCategoryTop10SessionID2CountRDD: RDD[((String, Int), List[(String, Int)])] = sessionID2RecordsRDD
      .mapPartitions(iter => {
        val bv: Array[String] = broadcastOfTop10CategoryID.value
        iter.flatMap {
          case (sessionID, records) => {
            // 求得有效的品类id和类型
  ...
  /**
    ...
    * @return
    */
  def getActionRDDByFilter(spark: SparkSession, taskParam: JSONObject): RDD[UserVisitSessionRecord] = {

    MockDataUtils.mockData(spark)
    // 过滤参数的获取
    val startDate: Option[String] = ParamUtils.getParam(taskParam, Constants.PARAM_START_DATE)
    val endDate: Option[String] = ParamUtils.getParam(taskParam, Constants.PARAM_END_DATE)
    val sex: Option[String] = ParamUtils.getParam(taskParam, Constants.PARAM_SEX)


    // 职业是一个多选的过滤参数，参数之间使用逗号分隔， eg: 程序员,教师,工人 ==> ui.professional IN ('程序员','教师','工人')
    //{"professional":"程序员，教师，工人"}
    val professionals: Option[String] = ParamUtils.getParam(taskParam, Constants.PARAM_PROFESSIONALS)
    val professionalWhereStr = professionals
      ...
      .getOrElse("")
    // 判断数据是否需要join操作
    val needJoinUserInfoTable: Option[Boolean] = if (sex.isDefined || professionals.isDefined) Some(true) else None
