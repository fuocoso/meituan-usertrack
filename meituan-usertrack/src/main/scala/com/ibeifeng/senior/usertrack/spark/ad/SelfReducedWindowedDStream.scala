...
class SelfReducedWindowedDStream[K: ClassTag, V: ClassTag](
                                                        parent: DStream[(K, V)],
                                                        reduceFunc: (V, V) => V,
                                                        invReduceFunc: (V, V) => V,
  ...
  override def compute(validTime: Time): Option[RDD[(K, V)]] = {
    ...
    val handlePreWindowValueF = handlePreWindowValueFunc
    ...
    val mergeValues = (arrayOfValues: Array[Iterable[V]]) => {
      ...
      if (arrayOfValues(0).isEmpty) {
        ...
        // 对上一个批次的结果数据进行处理操作
        tempValue = handlePreWindowValueF(tempValue)
...
object SelfReducedWindowedDStream {
  def reduceByKeyAndWindow[K: ClassTag, V: ClassTag](
    ...
    val cleanedHandlePreWindowValueFunc = ssc.sc.clean(handlePreWindowValueFunc)
    val cleanedFilterFunc = if (filterFunc != null) Some(ssc.sc.clean(filterFunc)) else None
    val partitioner = dstream.defaultPartitioner()
    new SelfReducedWindowedDStream[K, V](
      dstream, cleanedReduceFunc, cleanedInvReduceFunc, cleanedFilterFunc,
      windowDuration, slideDuration, partitioner, cleanedHandlePreWindowValueFunc
    )
  }
