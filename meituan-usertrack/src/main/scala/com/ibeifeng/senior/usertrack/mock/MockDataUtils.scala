...
object MockDataUtils {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val userVisitActionRDD: RDD[UserVisitAction] = mockUserVisitAction(sc)
  ...
  def mockUserInfo(sc: SparkContext): RDD[UserInfo] = {
    ...
    //  val tuple: (Int, String): = citys(random.nextInt(cityTypeSize))
  ...
  def mockUserVisitAction(sc: SparkContext): RDD[UserVisitAction] = {
    ...
    //userVisitSessionNumbers=500000
    for (i <- 0 until sessionNumbers) {
      ...
      //citys = ((0,shanghai),(1,tianjian),(2,shaghai),(3,chongqing))
  ...
  def mockProductInfo(sc: SparkContext): RDD[ProductInfo] = {
    // 1. 创建Data数据  1 product_1 {"product_type":"0"}
  ...
  //def loadProductInfoMockData(sc: SparkContext, sqlContext: SQLContext): Unit = {
    ...
    val productInfoRDD: RDD[ProductInfo] = {
      if (!fs.exists(productInfoDataSavePath)) {
        // 数据不存在，创建并返回rdd
        val rdd = mockProductInfo(spark.sparkContext)
        rdd.map(_.formatted()).saveAsTextFile(productInfoDataSavePathStr)
        // 返回
        rdd
      } else {
        // 读取数据
        val rdd = spark.sparkContext.textFile(productInfoDataSavePathStr)
        // rdd转换并返回
 ...
 // def mockData(sc: SparkContext, sqlContext: SQLContext): Unit = {
    val fs = FileSystem.get(spark.sparkContext.hadoopConfiguration)
     val userInfoDataSavePathStr = "/spark/project/data/mock/user_info"
      ...
      val userVisitActionSavePathStr = "/spark/project/data/mock/user_visit_action"
      val userVisitActionSavePath = new Path(userVisitActionSavePathStr)

      if (!fs.exists(userInfoDataSavePath)) {
      // 数据不存在
      val userInfoRDD = mockUserInfo(spark.sparkContext)
    ...
    if (!fs.exists(userVisitActionSavePath)) {
      // 数据不存在
      val userVisitActionRDD = mockUserVisitAction(spark.sparkContext)
    ...
    val userVisitActionRDD: RDD[String] = spark.sparkContext.textFile(userVisitActionSavePathStr)
