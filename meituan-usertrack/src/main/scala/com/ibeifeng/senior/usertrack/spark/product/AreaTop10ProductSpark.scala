...
import com.ibeifeng.senior.usertrack.spark.util.{SQLContextUtil, SparkConfUtil, SparkContextUtil, SparkSessionUtils}
...
//    )
    ...
    val spark = SparkSessionUtils.getInstance(
      conf,
      integratedHive = false
//      generateMockData = (spark)=>{
//        if(isLocal){
//          MockDataUtils.mockData(sc,spark)
//          MockDataUtils.loadProductInfoMockData(spark)
//        }
//      }
    )
    ...
    spark.udf.register("isNotEmpty", (str: String) => !isEmpty(str))
    spark.udf.register("concat_id_name", (id: Int, name: String) => {
      s"${id}:${name}"
    })
    spark.udf.register("fetch_product_type_from_json", (json: String) => {
      val value = JSON
        .parseObject(json)
    ...
    // 自定义UDAF
    spark.udf.register("concat_distinct_str", GroupConcatDistinctUDAF)

    // 三、数据过滤（不需要返回RDD以及过滤参数有可能不同）
    this.filterDataByTaskParamAndRegisterTmpTable(spark, taskParam)

    // 四、读取关系型数据库中的city_info城市信息表的数据，并注册成为临时表
    this.fetchCityInfoDataAndRegisterTmpTable(spark)

    // 五、将基础的用户访问数据和城市信息表数据进行join操作，并注册成为临时表
    this.registerTableMergeUserVisitActionAndCityInfoData(spark)

    // 六、统计各个区域、各个商品的点击次数(area、product_id、click_count)
    this.registerAreaProductClickCountTable(spark)

    // 七、分组排序TopN ==> 对各个区域的点击数据进行分组，然后对每组数据获取点击次数最多的前10个商品
    this.registerAreaTop10ProductClickCountTable(spark)

    // 八、合并商品信息，得到最终的结果表
    this.registerMergeTop10AndProductInfoTable(spark)

    // 九、结果输出
    this.saveResult(spark)
  }
  ...
  /**
    ...
    * @param spark
    */
  def saveResult(spark: SparkSession): Unit = {
    // 一般数据输输出到关系型数据库中
    /**
      * 数据输出到关系型数据库的方式：
  ...
  /**
    ...
    * @param spark
    */
  def registerMergeTop10AndProductInfoTable(spark:SparkSession): Unit = {
    MockDataUtils.loadProductInfoMockData(spark)
    // 1. hql的编写
    val hql =
    """
    ...
    // 3. 临时表注册
    df.createTempView("tmp_result")
      //.registerTempTable("tmp_result")
  }
  ...
  /**
    ...
    * @param spark
    */
  def registerAreaTop10ProductClickCountTable(spark: SparkSession): Unit = {
    ...
    """.stripMargin
    // 2. hql执行并注册临时表
    spark.sql(hql).registerTempTable("tmp_area_product_click_count_rnk")
    // 3. topN的hql编写
    val hql2 =
    """
      | SELECT
    ...
    """.stripMargin
    // 4. hql执行并注册临时表
    spark.sql(hql2).createTempView("tmp_area_top10_product_click_count")
  ...
  /**
    ...
    * @param spark
    */
  def registerAreaProductClickCountTable(spark: SparkSession): Unit = {
    /** *
      * 原始数据:
      * p1 c1 上海 华东
      * p1 c2 杭州 华东
      ...
      * 华北 p1 1 c5:北京:1
      * .....
      *  concat_id_name(city_id, city_name)   c1:上海  c5:北京   => c1:上海:20  c5:北京：12
  ...
  /**
    ...
    * @param spark
    */
  def registerTableMergeUserVisitActionAndCityInfoData(spark: SparkSession): Unit = {
    // 1. hql
    val hql =
    """
      | SELECT
   ...
   // df.registerTempTable("tmp_basic_product_city_info")
    df.createTempView("tmp_basic_product_city_info")
  }
  ...
  /**
    ...
    * @param spark
    */
  def fetchCityInfoDataAndRegisterTmpTable(spark:SparkSession): Unit = {
    // JDBC数据库的数据读取方式：通过SparkSQL的read模式进行读取, jdbc的数据读取接口总共有三个，一般使用自定义分区的方式来进行数据读取操作 ==> 这里使用最简单的方式
    spark
      .read
      .jdbc(url, table, properties)
        .createTempView("tmp_city_info")
  ...
  /**
    ...
    * @param taskParam
    */
  def filterDataByTaskParamAndRegisterTmpTable(spark:SparkSession, taskParam: JSONObject): Unit = {
  //def filterDataByTaskParamAndRegisterTmpTable(sqlContext: SQLContext, taskParam: JSONObject): Unit = {
   ...
   // df.registerTempTable("tmp_basic_user_visit_action")
    df.createTempView("tmp_basic_user_visit_action")
   // sqlContext.sql("select * from tmp_basic_user_visit_action limit 5").show()
    sql("select * from tmp_basic_user_visit_action limit 5").show()
