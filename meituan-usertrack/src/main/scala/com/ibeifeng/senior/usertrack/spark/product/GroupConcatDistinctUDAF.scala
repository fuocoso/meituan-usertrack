...
object GroupConcatDistinctUDAF extends UserDefinedAggregateFunction {
  ...
  /**
    ...
    * @param count  value对应出现的次数, 默认的出现次数
    * @return
    */
  private def mergeValue(buffer: String, value: String, count: Int = 1): String = {
    // 如果buffer中不包含value，直接在buffer的尾部添加一个新的个体即可；如果包含，找到对应的个体，然后将其次数增加一次  buffer->6:长沙:4   value->6:长沙
    if (buffer.contains(value)) {
      // 更新操作
      buffer
        .split(delimiter)   //Array("6:长沙:4")
        .map(v => {
      ...
      if ("".equals(buffer)) {
        s"${value}:${count}"  //value=c5:北京:1  count=1  =>c5:北京:1
      } else {
